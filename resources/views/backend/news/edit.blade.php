                ...
                <div class="row g-3">
                    <div class="col-lg-6">
                        <div class="form-floating">
                            <input required type="text" name="title" value="{{ $news->title }}" class="form-control" id="firstnamefloatingInput" placeholder="Enter your firstname">
                    ...
                    <div class="col-lg-6">
                       <div class="row">
                        <div class="col-lg-4"> 
                             ...
                             <select required name="category_id" class="form-select rounded-pill mb-3" aria-label="Default select example">
                            <option disabled selected="">Search for Category</option>
                            @foreach ($category as $category)
                            <option  value="{{ $category->id }}" {{ $category->id === $news->category->id ?'selected':'' }}> {{ $category->cName }}</option>
                        ...
                        <div class="col-lg-4"> 
                             <select required name="sub_category_id" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                <option disabled selected="">Search for SubCategory</option>
                                @foreach ($subcategory as $subcategory)
                            <option value="{{ $subcategory->id }}" {{ $subcategory->id === $news->sub_category_id ?'selected':'' }} >{{ $subcategory->sub_category_name }} </option>
                                
                            @endforeach
                            </select>
                        </div>
                        <div class="col-lg-4">
                              <select required name="division_id" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                <option disabled selected="">Search for Division</option>

                            @foreach ($division as $division)
                            <option value="{{ $division->id }}" {{ $division->id === $news->division->id ?'selected':'' }}>{{ $division->div_name }}</option>
                            @endforeach
                            </select>
                        </div>
                       </div>
                    ...
                    <div class="col-lg-8">
                            ...
                            <textarea required name="nBody" id="summernote" cols="30" rows="10" >
                                {{ $news->nBody }}
                            </textarea>
                    ...
                    <div class="col-lg-4">
                        <div class="row">
                            <div class="col-lg-6"> 
                                 <select required name="district_id" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                <option  disabled selected="">Search for District</option>
                                @foreach ($district as $district)
                                <option value="{{ $district->id }}" {{ $district->id === $news->district_id ?'selected':'' }}>{{ $district->dist_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-lg-6">  
                            <select required name="upazila_id" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                <option disabled selected="">Search for Upazila</option>
                                @foreach ($upazila as $upazila)
                                <option value="{{ $upazila->id }}" {{ $upazila->id === $news->upazila_id ?'selected':'' }}>{{ $upazila->upazila_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        </div>
                    ...
                    <div class="col-lg-4">
                        <div class="row">
                            <div class="col-lg-6">  
                                <input required name="nCaption" value="{{ $news->nCaption }}" type="text" class="form-control" id="firstnamefloatingInput" placeholder="Image Caption">
                                <label for="firstnamefloatingInput">Image Caption</label>
                            </div>
                            <div class="col-lg-6">  
                                <input name="ntag" value="{{ $news->nTag }}" type="text" class="form-control" id="firstnamefloatingInput" placeholder="Image Tag">
                    ...
                    <div class="col-lg-4">
                            ...
                            <div class="row">
                                ...
                                <option value="{{ $featured->id }}" {{ $featured->id === $news->featured_id ?'selected':'' }}>{{ $featured->featured}}</option>
                                @endforeach
                                    </select>
                                </div>
                                <div class="col-lg-4">  
                                    <select name="topic_id" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                        ...
                                        <option value="{{ $topic->id }}" {{ $topic->id === $news->topic_id ?'selected':'' }}>{{ $topic->topic}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-lg-4">  
                                    <select name="status" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                        ...
                                        <option {{ $news->status==='published'?'selected':'' }} value="published">Publish</option>
                                        <option {{ $news->status==='unpublished'?'selected':'' }} value="unpublished">Unpublish</option>
                        ...
                        <div class="row">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div>
                                        <img src="{{ asset($news->image) }}" width="50px" alt="news image picture">
                                    </div>
                                </div>
                                </div>
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-check form-check-outline form-check-warning mb-3">
                                    <select name="scroll" class="form-select rounded-pill mb-3" aria-label="Default select example">
                                        ...
                                        <option {{ $news->scroll===1?'selected':'' }} value="1">Yes</option>
                                        <option {{ $news->scroll===0?'selected':'' }} value="0">No</option>
